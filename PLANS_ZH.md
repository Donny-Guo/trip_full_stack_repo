...
授权历史：最初规划轮次只产出文档。2026-07-30 的请求单独授权远程创建 `ISSUE-001` 至 `ISSUE-027`，以及相应 Labels、Milestone、Assignee 和原生依赖。2026-08-02，Owner 明确授权计划内的首个本地认证切片，包括代码与脚手架、依赖与根锁文件、Hook、MIT License 治理、首切片 GitHub CI/治理配置、迁移、本地 PostgreSQL/pgvector 基础设施，以及同步文档/状态更新。2026-08-03，Owner 明确授权 D-25 及隔离的 Next.js/MUI/TypeORM Baseline Change。2026-08-05，Owner 授权 D-26：保留核心本地 Auth Path 的持久控制，同时把 ISSUE-007 之后全部实施压缩为 ISSUE-009/MVP-01 与 ISSUE-010/MVP-02，并把低优先级宽度移入本文档 Backlog。Owner 同时明确授权改写远程 #9/#10、关闭 #11-#27，并保持 #7 不变；该一次性远程整理已完成。2026-08-06，Owner 从 ISSUE-009 移除 Compromised/Common-password Screening，并用聚焦 MVP Evidence 取代穷举 Matrix 与正式 Argon2 Capacity Benchmark。随后 ISSUE-009 合并，#9 按 completed 关闭。Owner 还明确 D-26：ISSUE-010 只保留精简 Protected Dashboard 目标，并另行授权已经完成的对应远程 #10 编辑与同步文档。Dashboard Product Detail 移入 W-09。F-05 证明底层数据库 Role Boundary；已完成的 ISSUE-009 负责实际 NestJS/TypeORM Connection、首个 Migration 与认证 API。
...
完成一个可测试的最小纵向切片：开发者能在 pnpm Monorepo 中于宿主机启动 Web/API，并通过容器启动本地 PostgreSQL + pgvector；用户能注册、收到真实 API 成功反馈、进入并刷新刻意精简的受保护 Dashboard 目标、退出，并使用同一组凭据再次登录。
...
- 用于证明认证边界的刻意精简受保护 Dashboard Route，只包含可访问的 Route Identity 与认证路径所需 Logout Control。
- 可访问的英文 Sign Up/Login UI 与最小 Protected Dashboard 目标；完整本地化架构延期。
...
- Product Dashboard 内容或设计，包括 Card、Metric、Trip/Itinerary Data、Personalization、Responsive Composition 与精修 Visual。
...
D-01 至 D-26 全部已确认并作为当前权威。D-14 至 D-19 是已接受的独立审计建议；D-20 至 D-24 记录用户提出的 GitHub Identity、治理、CI/CD、本地 Hook、License 和 AI Review 要求；D-25 记录 Owner 授权的 Framework/ORM Major-line Revision；D-26 记录 Owner 授权的限时 Issue/Scope 合并，以及 2026-08-06 对 ISSUE-010 Dashboard 仅为精简 Protected Auth Destination 的明确说明。首切片实施在既定排除项内已获授权。
...
| D-08 | `CONFIRMED` | 首版密码为 8–20 个 ASCII 字符，只允许 `A-Z`、`a-z`、`0-9` 和 `$#@%`，且至少包含一个大写字母、一个小写字母、一个数字和一个 `$#@%` 字符；不 trim；只存储带 Salt 的 Argon2id Hash 并限制本地并发。本地 MVP 明确不含 Compromised/Common-password Screening 与正式 Capacity Benchmark                                                                                                                                                                                                                                   | B-04、B-06、W-04、R-13                          |
...
| D-26 | `CONFIRMED` | 对限时本地演示，保留 PostgreSQL-backed Auth API、JWT-cookie Session、可访问 Sign Up/Login/Protected-route Journey、关键 Test、本地 Hook 与最小 PR CI 的生产级要求。ISSUE-007 之后仅包装为 ISSUE-009/MVP-01 与 ISSUE-010/MVP-02。ISSUE-010 的 Dashboard 刻意精简，只含可访问 Route Identity 与 Logout；Product Content/Design 归 W-09 Backlog。Localization、Extensible Navigation、完整 GitHub Governance/Security Administration 与其他产品/发布宽度同样延期。合并不授权公开暴露，也不豁免保留的 Security Control | ISSUE-009、ISSUE-010、F-08、W-03、W-09、R-01 起 |
...
| 注册/重置/改密 password | 必填、8–20，显示字符类别和允许字符清单    | 只允许 `A-Z`、`a-z`、`0-9`、`$#@%`；四类各至少一个；不 trim                        | 允许粘贴和密码管理器；Compromised-password Screening 不在本地 MVP |
...
- 前置：D-08
- 动作：集中实现已确认的 8–20 ASCII 允许集合和四类字符规则；为每个失败原因定义稳定错误码；使用所选参数封装 Argon2id Hash/Verify 并限制并发成本。本 MVP 不添加本地或远程 Compromised-password Screening。
- 产出：可替换、可测试的 `PasswordPolicy` 和 `PasswordHasher`。
- 验收：聚焦覆盖 7/8/20/21 边界、每一种缺失类别、允许的特殊字符及代表性不支持/空白/Unicode 输入，并证明粘贴输入不被改写；同一密码产生不同 Salt Hash；正确/错误密码验证符合预期；明文和 Hash 不进入 Response/Log；所选算法、参数、Rehash 行为与有界并发有记录。
...
- 动作：创建 DTO、全局输入校验、规范邮箱、密码策略检查、密码哈希和 User 创建；映射校验/重复错误；成功后复用 TokenIssuer，并返回冻结的安全 User + `messageCode` 契约。
- 产出：`POST /api/v1/auth/sign-up`。
- 验收：成功 `201` 并建立真实登录态；聚焦格式/长度/缺失类别/不允许字符/未知字段返回安全、可定位的 `400`；重复邮箱为 `409`；Response/Log 无 Hash/明文/Token；并发重复注册只成功一次。
...
- 产出：独立、可重复的 API 测试套件。
- 验收：聚焦覆盖注册/登录/会话成功、Canonical Duplicate 与并发注册、未知用户 Dummy-hash、JWT Signature/Algorithm/必需 Claim/过期失败、Cookie 设置/删除一致性、未知字段、不支持 Content Type、过大 Body、受信/不受信/缺失 Provenance、No-store、无效 Token、Runtime/Migrator Boundary 与敏感字段/日志泄露；测试不依赖顺序。
...
- 产出：可用的 `/sign-up`。
- 验收：空值、非法邮箱及每一项现役密码规则都获得即时、可访问反馈；`409` 服务端错误清晰；API 成功设置 Cookie 后自动进入 Dashboard；键盘提交、粘贴、密码管理器、焦点和错误播报可用；不记录密码或读取 Token。
...
- 前置：W-06、W-07
- 动作：测试 Auth Form State、密码规则清单、API Error Mapping、Protected-route/Session/Logout Behavior、自动 Accessibility 与 Keyboard Flow；Product Dashboard、Navigation 与 Localization Matrix 留待后续。
- 产出：Web 测试套件与手测记录。
- 验收：关键交互不只由实现细节断言；无严重可访问性违规；Sign Up/Login 覆盖窄屏和桌面宽度；精简 Dashboard 保持可访问、可用，但不要求 Product-content Visual Matrix。
...
- 验收：注册成功进入 Dashboard；密码规则、重复邮箱、来源失败和 API 故障符合契约；Gateway 不缓存私有/`Set-Cookie` 响应；数据库无明文密码；Web 不可读取 JWT。
...
以下任务只有在首个纵向切片验收后或获得单独明确授权后才启动。F-08 完整 Repository Governance、W-03 Extensible Navigation、W-09 Product Dashboard 与已延期 Localization Boundary 与 R 系列一起进入本文档 Backlog。每项都应独立 Review、迁移、测试和发布；列出不代表实施授权或远程 Issue 创建授权。

#### W-09 定义并构建 Product Dashboard Experience — `TODO（后续）`

- 前置：H-01，且 Dashboard User Outcome 与 Data Source 已单独批准；若该 Experience 需要 Product Navigation，则前置 W-03
- 动作：先定义 Dashboard 的真实 User Job，再选择 Content/Layout；随后仅实现已批准的 Product Data、Card/Metric、Trip/Itinerary Summary、Personalization、Empty/Loading/Error State、Responsive Composition 与 Visual Design。不得从 ISSUE-010 Auth Demo 推断这些要求。
- 验收：已批准 Product Specification 说明每个 Data Source 与 State；实现可访问、响应式，不削弱 Protected-route Boundary，并具备与真实交互相称的 Component/E2E Coverage。
...
- 动作：在不记录密码下复审放弃率和支持数据；考虑更长上限、更广字符、Passphrase、移除组合规则，以及公开暴露前是否有依据增加本地 Compromised-password Screening；保留 Argon2id 并同步英文契约和 `_ZH`。
...
3. **ISSUE-010 Local Demo**：同源 Web Boundary 与最小 Auth Route，包括刻意精简的 Protected Dashboard 目标；Sign Up/Login/Session；Hook 与 PR CI；Component/E2E Verification 与同步交接文档。
...
| 已选择的 8–20 组合/允许集合规则                              | 拒绝许多密码管理器结果和长 passphrase，并可能诱导可预测的合规模式        | 一次性清晰清单、允许粘贴/自动填充、集中策略、Argon2id 存储和明确 R-13 复审路径                                                                                |
...
- D-26 的两个执行 Issue 合并与文档化延期（包括精简 ISSUE-010 Dashboard Boundary）已接受。
- 英文权威和 `_ZH` 跟随规则已接受。

Owner 已于 2026-08-02 发出独立实施开始指令。2026-08-05，Owner 进一步授权 D-26，以及一次性远程改写 #9/#10、关闭 #11-#27，同时排除 #7；这些远程操作已经完成。2026-08-06，Owner 另行授权把远程 #10 收窄到精简 Protected Dashboard Boundary，并同步这些文档。以上授权均不允许 Post-MVP 实施、生产部署、启用 CD、Cloud Resource、公开暴露、仓库 Visibility 变更、远程创建 `ISSUE-028` 及之后的 Issue，或没有新明确请求的任何其他远程 Issue Mutation。
