...
Decisions D-01 through D-26 are confirmed. On 2026-08-02, the owner authorized the planned first local authentication slice, closing `P-02`/`ISSUE-001`. The authorization covers planned code/scaffolding, dependencies and the root lockfile, hooks, MIT-license governance, first-slice GitHub CI/governance, migrations, local PostgreSQL/pgvector infrastructure, and synchronized documentation/status updates. `P-03`/`ISSUE-002`, `F-01` through `F-05`/`ISSUE-003` through `ISSUE-007`, `W-01`/`ISSUE-008`, and `ISSUE-009`/`MVP-01` are complete locally; #9 closed as completed after its implementation merged on 2026-08-06. On 2026-08-05, D-26 compressed the time-limited local demo without weakening its retained core controls: implement only `ISSUE-009`/`MVP-01` and then `ISSUE-010`/`MVP-02`. GitHub issues #11 through #27 are closed as superseded or deferred, not as implementation evidence. F-05 proves a clean migration-shaped transactional DDL probe and direct TCP/SCRAM runtime-role behavior with `psql`; B-01 retains the actual NestJS/TypeORM connection and readiness boundary, and B-02 retains the first real application migration.

On 2026-08-06, the owner narrowed `ISSUE-009` to a pragmatic local MVP and explicitly removed compromised/common-password screening and its vendored blocklist assets from that issue. Focused acceptance evidence replaces exhaustive matrices and formal Argon2 capacity benchmarking for the local MVP. Public exposure remains separately gated.

On 2026-08-06, the owner also clarified D-26 and `ISSUE-010`: `/dashboard` is only the intentionally sparse protected destination needed to prove the authentication journey. ISSUE-010 requires accessible route identity and the logout control, but no dashboard cards, metrics, trip data, product navigation, personalization, or polished product design. Those details belong to later `W-09` work and require separate authorization.

The ISSUE-010 implementation and local verification completed on 2026-08-06, including the minimal Web authentication journey, session-aware 404 behavior, local hooks, and PR CI configuration. GitHub #10 remains open pending normal PR CI, review, merge, and remote closure; local completion is not public-release approval.
...
That authorization excludes post-MVP implementation, production deployment, CD activation, cloud resources, public exposure, repository-visibility changes, and remote creation of `ISSUE-028` onward. The owner separately authorized remote creation of `ISSUE-001` through `ISSUE-027` and their scoped metadata on 2026-07-30. On 2026-08-05, the owner explicitly authorized the now-completed rewrite of #9/#10 and closure of #11-#27 while excluding #7. On 2026-08-06, the owner separately authorized the now-completed narrowing of remote #10 and synchronized documentation. Any other remote issue operation needs a separate explicit request.
...
- The public repository belongs to `@Donny-Guo`; MIT is confirmed. The tracked root `LICENSE` notice conflicts with D-23: preserve it until the deferred F-08 governance change aligns and verifies it reviewably.
...
- When W-03 product navigation begins later, it uses semantic links and supports keyboard access, active routes, mobile layouts, future permission filtering, and feature flags. ISSUE-010 needs only the minimal Sign Up/Login/protected Dashboard route flow. Keep Dashboard intentionally sparse: accessible page identity plus the logout control required by the auth journey. Product Dashboard content and design belong to later W-09.
...
- Initial UI copy is English. Under D-26, keep auth/demo copy cohesive within its feature without building the deferred catalog/localization framework; R-08 later owns stable catalog keys, locale-aware formatters, persistence, and the first additional locale (`zh-CN`). No locale router or switcher belongs in ISSUE-010.
...
- The local MVP does not implement compromised/common-password screening, a vendored blocklist, or a remote password lookup. Reconsider screening only through a separately approved pre-public hardening decision; never send a candidate password or derived hash to a remote lookup.
...
- Component: auth form states, accessible errors, API-error mapping, and protected-route/session behavior; responsive navigation and product Dashboard coverage begin with later W-03/W-09.
- E2E: sign-up auto-login, duplicate registration, restore, logout/login, invalid credentials, and protected routes.
