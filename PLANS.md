...
Status: **decisions D-01 through D-26 confirmed; first-slice implementation authorized on 2026-08-02, consolidated on 2026-08-05, and narrowed on 2026-08-06; P-03, F-01 through F-05, W-01, and ISSUE-009 are complete; ISSUE-010 is implemented and verified locally but remains open pending PR CI, review, merge, and remote closure**\
...
Authorization history: the original planning round created documentation only. A 2026-07-30 request separately authorized remote creation of `ISSUE-001` through `ISSUE-027` plus their labels, milestone, assignment, and native dependencies. On 2026-08-02, the owner explicitly authorized the planned first local authentication slice, including code and scaffolding, dependencies and the root lockfile, hooks, MIT license governance, first-slice GitHub CI/governance configuration, migrations, local PostgreSQL/pgvector infrastructure, and synchronized documentation/status updates. On 2026-08-03, the owner explicitly authorized D-25 and the isolated Next.js/MUI/TypeORM baseline change. On 2026-08-05, the owner authorized D-26: retain the durable controls on the core local auth path while consolidating all work after ISSUE-007 into ISSUE-009/MVP-01 and ISSUE-010/MVP-02, moving lower-priority breadth to this documented backlog. The owner also explicitly authorized rewriting remote issues #9/#10 and closing #11-#27 while leaving #7 untouched; that one-time remote reorganization is complete. On 2026-08-06, the owner removed compromised/common-password screening from ISSUE-009 and replaced exhaustive matrices and formal Argon2 capacity benchmarking with focused MVP evidence. ISSUE-009 then merged and #9 closed as completed. The owner also clarified D-26 so ISSUE-010 keeps only a sparse protected Dashboard destination and separately authorized the now-completed matching remote #10 edit plus synchronized documentation. Dashboard product details move to W-09. F-05 proves the underlying database role boundary; completed ISSUE-009 owns the actual NestJS/TypeORM connection, first migration, and authentication API.

Authorization exclusions: post-MVP implementation, production deployment, CD activation, cloud resources, public exposure, repository visibility changes, remote creation of `ISSUE-028` onward, and any remote issue operation beyond the completed 2026-08-05 reorganization and the owner-authorized 2026-08-06 narrowing of #10 still require separate explicit authorization.

Current repository state: `Donny-Guo/trip_full_stack_repo` is public. The root pnpm/Turborepo workspace, one pnpm lockfile, editor/ignore conventions, local-only task graph, and independently buildable `apps/web` and `apps/api` applications exist. One root Prettier 3.9.6 policy and standardized root checks are active; narrow shared strict TypeScript and typed ESLint packages are consumed by both applications. The Web application runs Next.js 16.2.12 with React 19.2.8 and the MUI v9 App Router SSR/CSS-variable theme foundation; ISSUE-010 adds sign-up/login, a server-protected sparse Dashboard, session restore/logout, a session-aware cancelable 404 redirect, focused component/E2E tests, local hooks, and immutable-SHA-pinned PR CI configuration. The merged ISSUE-009 implementation provides NestJS 11.1.28 ESM authentication, TypeORM 1.1 runtime/migration data sources, the first `users` migration, PostgreSQL-backed readiness, Argon2id, JWT/cookie sessions, focused unit tests, and real-PostgreSQL integration tests. Digest-pinned local PostgreSQL 18.4/pgvector 0.8.5 infrastructure provides a loopback-only healthy service and separated provisioner/migrator/runtime roles. Remote PR CI/review/merge evidence, agent capabilities, and other business features remain outstanding. A tracked root MIT `LICENSE` predates this authorization; its current notice awaits F-08 alignment with D-23.
...
Deliver a testable vertical slice in which developers run Web/API on the host through the pnpm monorepo, start PostgreSQL + pgvector as local containerized infrastructure, and complete this user journey: register, receive real API success feedback, enter and refresh an intentionally sparse protected Dashboard destination, log out, and log in again with the same credentials.
...
- Intentionally sparse protected Dashboard route proving the authenticated boundary, with only accessible route identity and the logout control required by the journey.
- Accessible English Sign Up/Login UI and the minimal protected Dashboard destination; full localization architecture is deferred.
...
- Product Dashboard content or design, including cards, metrics, trip/itinerary data, personalization, responsive composition, and polished visuals.
...
| D-08 | `CONFIRMED` | Initial password policy: 8–20 ASCII characters, allowed set `A-Z`, `a-z`, `0-9`, and `$#@%`, with at least one uppercase, lowercase, digit, and `$#@%` character; never trim; store only salted Argon2id hashes with bounded local concurrency. The local MVP deliberately omits compromised/common-password screening and formal capacity benchmarking                                                                                                                                                                                                                                                                                                                     | B-04, B-06, W-04, R-13                              |
...
| D-26 | `CONFIRMED` | For the time-limited local demo, keep production-quality requirements on the PostgreSQL-backed auth API, JWT-cookie session, accessible Sign Up/Login/protected-route journey, critical tests, local hooks, and minimum PR CI. After ISSUE-007, package this work only as ISSUE-009/MVP-01 and ISSUE-010/MVP-02. In ISSUE-010, Dashboard is intentionally sparse: accessible route identity and logout only; product content and design are W-09 backlog. Defer localization, extensible navigation, full GitHub governance/security administration, and other product/release breadth. Consolidation never authorizes public exposure or waives retained security controls | ISSUE-009, ISSUE-010, F-08, W-03, W-09, R-01 onward |
...
| password on sign-up/reset/change | Required; 8–20; show category/allowed-character checklist     | Enforce `A-Z`, `a-z`, `0-9`, `$#@%` only; at least one of each category; never trim                                                  | Paste and password managers remain enabled; compromised-password screening is outside the local MVP    |
...
- Prerequisite: D-08
- Action: centralize the confirmed 8–20 ASCII allowed-set and category rules; define stable error codes per failed requirement; wrap Argon2id hash/verify with the selected parameters and bound concurrent cost. Do not add local or remote compromised-password screening in this MVP.
- Output: independently testable `PasswordPolicy` and `PasswordHasher` boundaries.
- Acceptance: focused 7/8/20/21 boundaries, each missing category, allowed special characters, representative unsupported/whitespace/Unicode input, and unchanged paste input are tested; the same password produces different salted hashes; correct/incorrect verification works; plaintext/hash never enter responses or logs; selected algorithm, parameters, rehash behavior, and bounded concurrency are documented.
...
- Action: create DTO, global validation, canonical email normalization, password-policy checks, hashing, and User creation; map validation and duplicate errors; reuse TokenIssuer on success and return the frozen safe User plus `messageCode` contract.
- Output: `POST /api/v1/auth/sign-up`.
- Acceptance: success returns `201` and a real session; focused format/length/category/unsupported-character/unknown-field cases return safe, actionable `400`; duplicate returns `409`; responses/logs contain no password, hash, or token; one concurrent duplicate registration succeeds.
...
- Acceptance: focused tests cover sign-up/login/session success, canonical duplicate and concurrent registration, dummy-hash unknown-user behavior, JWT signature/algorithm/required-claim/expiry failures, cookie set/delete parity, unknown fields, unsupported content types, oversized bodies, trusted/untrusted/missing provenance, no-store headers, invalid tokens, runtime/migrator boundaries, and sensitive-field/log leakage; tests remain order independent.
...
- Prerequisite: W-01
- Action: create `(auth)` and `(app)` route groups for `/sign-up`, `/login`, and protected `/dashboard`; exclude protected content from auth pages. Keep Dashboard to accessible route identity and the logout control needed by the auth journey. Defer product Dashboard content/design to W-09 and extensible navigation, Flight Info/User targets, and the localization catalog boundary under D-26.
- Output: minimal Login and Sign Up routes plus an intentionally sparse protected Dashboard destination.
- Acceptance: route-group names are absent from URLs; direct/refresh navigation works; loading/error/404 boundaries are clear; an authenticated 404 offers a cancelable countdown to Dashboard, an unauthenticated 404 offers one to Sign In, and an unavailable session check does not redirect; `html lang` matches the English demo; private content does not flash; Dashboard adds no cards, metrics, trip/itinerary data, product navigation, personalization, or polished product design.
...
- Acceptance: empty/invalid email and every active password rule receive immediate accessible feedback; the `409` server error is clear; success enters Dashboard after the cookie is set; keyboard submit, paste, autofill, focus, and error announcement work; no password/token logging.
...
- Action: test auth form states, password checklist, API error mapping, protected-route/session/logout behavior, automated accessibility, and keyboard flows. Product Dashboard, navigation, and localization matrices remain later work.
- Output: Web test suite and manual-check record.
- Acceptance: tests assert behavior rather than incidental implementation; no serious accessibility violations; Sign Up/Login cover narrow and desktop layouts; the sparse Dashboard remains accessible and functional without a product-content visual matrix.
...
- Acceptance: valid registration enters Dashboard; invalid password categories/characters, duplicate email, provenance failure, and API outage follow the contract; gateway does not cache private/`Set-Cookie` responses; database has no plaintext password; Web cannot read JWT.
...
Each item begins only after the first slice is accepted or receives separate explicit authorization. F-08 full repository governance, W-03 extensible navigation, W-09 product Dashboard work, and the deferred localization boundary join the R-series work in this documented backlog. Every item requires independent review, migrations where relevant, tests, and release evidence. Listing does not authorize implementation or remote issue creation.

#### W-09 Define and build the product Dashboard experience — `TODO (later)`

- Prerequisite: H-01 and separately approved Dashboard user outcomes and data sources; W-03 if the experience needs product navigation
- Action: define the Dashboard's real user jobs before choosing content or layout, then implement only approved product data, cards/metrics, trip or itinerary summaries, personalization, empty/loading/error states, responsive composition, and visual design. Do not infer these requirements from the ISSUE-010 auth demo.
- Acceptance: an approved product specification names each data source and state; the implementation is accessible and responsive, does not weaken the protected-route boundary, and has component/E2E coverage proportional to its real interactions.
...
- Action: review validation-abandonment and support data without logging passwords; consider longer maxima, broader characters, passphrases, removing composition requirements, and whether local compromised-password screening is justified before public exposure; retain Argon2id and update the English contract and `_ZH` follower together.
...
```text
Completed: P-01..P-03, F-01..F-05, W-01, ISSUE-007, ISSUE-009 / MVP-01
Local pass, remote open: ISSUE-010 / MVP-02
```

ISSUE-009 is complete. ISSUE-010's Web boundary, auth UI/session behavior, hooks/CI, tests, and documentation are implemented and locally verified in reviewable layers against the completed ISSUE-009 API. Remote PR CI, review, merge, and closure remain. A single owner coordinates primary English docs and `_ZH` followers to prevent drift.
...
3. **ISSUE-010 local demo**: same-origin Web boundary and minimal auth routes, including the intentionally sparse protected Dashboard destination; Sign Up/Login/session behavior; hooks and PR CI; component/E2E verification and synchronized handoff docs.
...
| Selected 8–20 composition/allowed-set policy                                | Rejects many password-manager values and long passphrases; may encourage predictable compliance patterns           | Clear single checklist, paste/autofill, centralized policy, Argon2id storage, and an explicit R-13 review path                                                                                                                                |
...
Confirmed in D-26: the time-limited local demo retains production-quality requirements on the core authentication path while consolidating post-infrastructure execution into ISSUE-009/MVP-01 and ISSUE-010/MVP-02. The 2026-08-06 clarification keeps ISSUE-010 Dashboard work to an intentionally sparse protected auth destination; product Dashboard content and design move to W-09. Localization, extensible navigation, and full GitHub governance/security administration are also documented later work; this scope compression does not change the public-release gate.

No foundational product, review, or first-slice implementation gate remains. The owner closed `P-02`, the version-policy evidence completed `P-03`, and the monorepo-root evidence completed `F-01` locally on 2026-08-02. `F-02` completed locally and was reverified on Next.js 16 on 2026-08-03; `F-03` completed locally with its verified NestJS lifecycle and liveness boundary on 2026-08-03. `F-04` completed locally with shared TypeScript/ESLint configuration, one root Prettier policy, standardized checks, negative-rule probes, and cache/ignore/package-boundary evidence on 2026-08-05. `F-05` completed locally on 2026-08-05 with digest-pinned PostgreSQL/pgvector infrastructure, atomic bootstrap, separated roles, clean-start/security/persistence evidence, and the owner-approved B-01/B-02 boundary. ISSUE-009/MVP-01 merged and #9 closed as completed on 2026-08-06. ISSUE-010/MVP-02 is implemented and verified locally but remains open for PR CI, review, merge, and remote closure. GitHub #11-#27 were closed as superseded or deferred, not as implementation evidence. The independent public-release gate remains blocked, and the first-slice authorization does not extend to later or production work.
...
- D-26 two-issue execution consolidation and documented deferrals, including the sparse ISSUE-010 Dashboard boundary, are accepted.
- English authority and `_ZH` follower rules are accepted.

The owner issued the separate implementation-start instruction on 2026-08-02. On 2026-08-05, the owner further authorized D-26 and the one-time remote rewrite of #9/#10 plus closure of #11-#27 while excluding #7. Those remote actions are complete. On 2026-08-06, the owner separately authorized narrowing remote #10 to the sparse protected Dashboard boundary and synchronizing these documents. None of these authorizations permits post-MVP implementation, production deployment, CD activation, cloud resources, public exposure, repository visibility changes, remote creation of `ISSUE-028` onward, or any other future remote issue mutation without another explicit request.
