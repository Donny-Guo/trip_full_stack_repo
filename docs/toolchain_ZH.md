...
- 下游 Enforcement：**F-01/ISSUE-003 已于 2026-08-02 在本地完成；F-02/ISSUE-004 已于 2026-08-03 在 Next.js 16 上完成并重新验证；W-01/ISSUE-008 已于 2026-08-05 在本地完成**

“已冻结”表示下游任务必须使用这些精确 Direct Selection；只有经 Review 的同一变更更新本矩阵后才能改动。D-25 仅取代原有 Next.js/MUI/TypeORM Selection；P-03 其余内容继续有效。P-03 Status 本身不声称 Installation、Lockfile、Build、Native-runtime、Database 或 CI Result 已存在；这些事实由下文指定的 Downstream Enforcement Task 证明。F-01 提供 Root Installation、Lockfile 与 Task-graph Evidence；F-02 提供已安装 Next.js Web Evidence；W-01 提供已安装 MUI SSR/Theme、Hydration、Navigation 与 Web Render-test Evidence。TypeORM/PostgreSQL Runtime Behavior、Native-module Loading、Database-image Verification 与 CI 仍由后续 Owner Task 负责。
...
| Component | 精确选择 | Compatibility 与 Support Evidence | Pin / Downstream Enforcement | Class | Primary Source |
| --- | --- | --- | --- | --- | --- |
| ESLint | `eslint@9.39.5`、`@eslint/js@9.39.5`、`typescript-eslint@8.65.0`、`eslint-config-prettier@10.1.8`、`globals@17.9.0` | Next 16 发布原生 Flat Config 并接受 ESLint `>=9`；F-02 直接导入 `core-web-vitals` 与 `typescript`，不再把 `@eslint/eslintrc`/`FlatCompat` 作为 Direct Compatibility Dependency。`typescript-eslint` 接受 TypeScript 5.9.3 与 ESLint 9。在 Shared F-04 Stack 证明兼容交集前，ESLint 10 仍明确排除。 | F-02 App-local 原生 Flat Config；F-04 Shared Flat Config；F-07 Staged Lint；F-06 Lint | R3 | [ESLint](https://www.npmjs.com/package/eslint/v/9.39.5)、[ESLint JS](https://www.npmjs.com/package/@eslint/js/v/9.39.5)、[Next ESLint Guide](https://nextjs.org/docs/app/api-reference/config/eslint)、[typescript-eslint](https://www.npmjs.com/package/typescript-eslint/v/8.65.0)、[config-prettier](https://www.npmjs.com/package/eslint-config-prettier/v/10.1.8)、[globals](https://www.npmjs.com/package/globals/v/17.9.0) |
...
| Web Unit/Component Test | `vitest@4.1.10`、`vite@8.2.0`、`@vitejs/plugin-react@6.0.5`、`jsdom@30.0.1`、`@testing-library/react@16.3.2`、`@testing-library/dom@10.4.1`、`@testing-library/user-event@14.6.1`、`@testing-library/jest-dom@7.0.0` | Vitest 接受 Node 24 与 Vite 8；jsdom 要求 Node `^24.15`，24.18.0 满足；Testing Library 接受 React 19。W-01 已安装 Render-test Subset；`user-event` 仍为已选版本，但在首个 Interaction Test 负责前不安装。Async Server Component 通过 E2E 覆盖，不强塞入 jsdom。 | W-01 App-local Config 与 Render Regression；F-04 Shared Config；后续 Interaction/Component Task；F-06 | R3 | [Vitest](https://www.npmjs.com/package/vitest/v/4.1.10)、[Vite](https://www.npmjs.com/package/vite/v/8.2.0)、[React Plugin](https://www.npmjs.com/package/@vitejs/plugin-react/v/6.0.5)、[jsdom](https://www.npmjs.com/package/jsdom/v/30.0.1)、[React Testing Library](https://www.npmjs.com/package/@testing-library/react/v/16.3.2)、[DOM Testing Library](https://www.npmjs.com/package/@testing-library/dom/v/10.4.1)、[user-event](https://www.npmjs.com/package/@testing-library/user-event/v/14.6.1)、[jest-dom](https://www.npmjs.com/package/@testing-library/jest-dom/v/7.0.0)、[Next Testing Guide](https://nextjs.org/docs/app/guides/testing) |
...
因此，中英文文档通过 Repository Documentation Check 后，P-03 已于 2026-08-02 作为 Version-policy Task 关闭。D-25 于 2026-08-03 修订其 Web/ORM Row；随后 F-02 中的 Next.js 16.2.12 通过 Install、Lint、Route Type Generation、Strict TypeScript 与 Production Build。W-01 于 2026-08-05 完成 MUI Runtime/SSR Evidence；TypeORM Runtime Evidence 仍明确 Pending 于 B-01。下游失败须通过经 Review 的 Matrix Update 重新打开相应 Compatibility Decision；绝不使用 `--force`、Legacy Peer Handling、Unpinned Replacement 或 Skipped Test 隐藏失败。
